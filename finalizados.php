<head>
    ...
    <title>Condominios Finalizados </title>
...
try{
    ...
    // , ser.id AS ser_id, st.id AS st_id, us.id AS us_id

    $stmt = $conn->prepare("SELECT *, inf.id AS inf_id, ser.id AS ser_id FROM informacoes inf
    JOIN servicos ser ON ser.id = inf.servico_id
    INNER JOIN statuses st ON st.id = inf.status_id
    INNER JOIN users us ON us.id = inf.user_id
    WHERE status_id='4'");
...
<input class="btn_sair" type="submit" name="submit" value="Voltar" onclick= "window.location.href='home.php'" >
